...
cam_x,cam_y,cam_spdx,cam_spdy,cam_gain,cam_offx,cam_offy,cam_zoom, --camera values <camtrigger>
_pal --for outlining
=
{},{},
0,0,0,0,-99,
...
player={
  ...
  update=function(_ENV)
    ...
    --<badeline_orb>--
    if flying>0 then
      if (flying%2 == 0) init_object(launch_effect,x+3,y+5)
      flying-=1
    end
    --</badeline_orb> --
    -- horizontal input
...
badeline_orb = {
  ...
  update = function(_ENV)
    local is_last = next_pos>#positions
    if can_launch then
      off += 0.02
      hit = player_here()
      if hit then
        ...
        -- set animation stuff
        anim, flip.x = is_last and 20 or 5, gplayer.x-_x<0

        hit.spd, hit.djump, hit.x, hit.y, hit.dash_time, hit.nocontrol, can_launch = 
        vector(0,0), max_djump, _x, y, 0, is_last, false

        -- focus cam to player
        if (is_last) _g.cam_focus = gplayer
        sfx"23"
      end

      -- Hover towards player
      if gplayer then
        dist = sqrt(((gplayer.x-x+8)^2+(gplayer.y-y+8)^2))
        pdir = atan2(gplayer.y-y+8,gplayer.x-x+8)
        if dist < 32 then
          x = appr(x,_x+sin(pdir)*4,1)
          y = appr(y,start+cos(pdir)*4,1)
        else
          x = appr(x,_x,1)
          y = appr(y,flr(start+sin(off)*2),1)
        end
      end
    elseif anim > 0 then
      sprite = 65
      anim -= 1
      x=_x+(flip.x and 4 or -4)

      -- zoom out
      if (is_last) _g.cam_zoom = appr(cam_zoom,1.5,0.05)

      -- end animation stuff
      if anim <= 0 then
        
        hit, freeze, hit.spd.x, hit.spd.y, hit.flying =
        nil, 2, 0, -3.5, not is_last and 6 or 99
...
launch_effect = {
  ...
  draw = function(_ENV)
    oval(x - size * 1.5, y - size/2, x + size * 1.5, y + size/2, 8-(size/6)*2)
...
function _draw()
  ...
  camera()
  if cam_zoom != 1 and cam_focus then
    local size = 128/cam_zoom
    -- set the screen memory as the spritesheet
    -- and stretch screen->screen
    palt(0)
    poke(0x5f54, 0x60)
    sspr(mid(cam_focus.x+4-size/2,0,lvl_pw-size),mid(cam_focus.y+4-size/2,0,lvl_ph-size), size,size, 0,0, 128,128) 
...

...

...

