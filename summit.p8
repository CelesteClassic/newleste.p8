...
player={
  ...
  update=function(this)
    ...
    --<badeline_orb>--
    if this.flying>0 then
      init_object(launch_effect,this.x+3,this.y+5)
      this.flying-=1
    end 
    --</badeline_orb> --
    -- horizontal input
...
badeline_orb = {
  ...
  update = function(this)
    this.off += 0.02
    this.y = flr(this.start + sin(this.off) * 2)
    local hit = this.player_here()
    if hit then
      freeze, hit.djump, hit.spd.x, hit.spd.y, hit.dash_time, hit.x, hit.y, hit.flying =
        2, max_djump, 0, -3.5, 0, this.x, this.y, this.spr == 30 and 99 or 3
      sfx(19)
      this.init_smoke()
      destroy_object(this)
    end
...
launch_effect = {
  ...
  draw = function(this)
    oval(this.x - this.size * 2, this.y - this.size, this.x + this.size * 2, this.y + this.size, this.size > 3 and 6 or 7)
