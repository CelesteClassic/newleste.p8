...
player={
  ...
  update=function(_ENV)
    ...
    -- spike collision / bottom death / stationary fuzzy
    if is_flag(0,0,-1) or
	    y>lvl_ph and not exit_bottom or is_flag(0,0,6) then
	    kill_player(_ENV)
    end
...
fuzz={
  ...
  init=function(_ENV)
    ...
    --eyes={axis=="x" and cos(rnd"128"+x) or 0,axis=="y" and sin(rnd"128"+y) or 0}
    eye_offset=0
...
still_fuzz={
  init=function(_ENV)
    --eyes={cos(rnd"128"+x),sin(rnd"128"+y)}
    collides=true
    hitbox=rectangle(0,0,8,8)
    if (check(fuzz_spawn,0,0)~=nil) then
      spawned=true
    else
      hitbox=rectangle(1,1,6,6)
      spawned = false
    end
...
fuzz_spawn={
  init=function(_ENV)
    --eyes={cos(rnd"128"+x),sin(rnd"128"+y)}
    outline=false
    hitbox=rectangle(0,4,8,4)
    layer=0
    player_was_here={false}
    seed=rnd"128"
    seed2=rnd"256"
  ...
  update=function(_ENV)
    ...
    for i=1,size do
      if player_was_here[i]!=-1 then
        ...
        local c = false
        if (p~=nil) c = ( dir_x==-1 or p.right()>=x + (dir_y~=0 and i*8-5 or dir_x==-1 and 4 or 0) ) and ( dir_y==1 or p.bottom()>=y+(dir_x~=0 and i*8-5 or dir_y==1 and 4 or 0) ) and ( dir_x==1 or p.left()<=x + (dir_y~=0 and (i+1)*8-5 or dir_x==-1 and 8 or 4) ) and ( dir_y==-1 or p.top()<=y+(dir_x~=0 and (i+1)*8-5 or dir_y==1 and 8 or 4) )

        if (c and not player_was_here[i]) player_was_here[i]=true
        if (player_was_here[i]==true and not c) player_was_here[i]=4
        if (player_was_here[i]!=false and player_was_here[i]!=true and player_was_here[i]>=0) player_was_here[i]-=0.5
  ...
  draw=function(_ENV)
    ...
    for i=1,size*2+1 do
      ...
      if pwh~=-1 then
        k=i*4-1
        for j=0,(pwh==false or pwh==true) and 2 or pwh-1 do
          -- wiggle wiggle wiggle wiggle wiggle, yeah
          if (j>0) k+=mid(-1,round(sin((j/3)*(i%2==0 and seed+(i*200) or seed2+(i*200) + time()/4))-0.4),1)
          pset(x+(dir_y~=0 and k or 0)+(1*abs(dir_y))+(j*dir_x)+(dir_x==-1 and 7 or 0),y+(dir_x~=0 and k or 0)+(dir_y==1 and 7 or 0)-(j*dir_y)+(1*abs(dir_x)),1) -- shading
          pset(x+(dir_y~=0 and k or 0)+(j*dir_x)+(dir_x==-1 and 7 or 0),y+(dir_x~=0 and k or 0)+(dir_y==1 and 7 or 0)-(j*dir_y),2) -- line
        end
      end
    end
    ...
    if (#fuzzy_drawer.fuzzies!=0) then
      ...
      for f in all(fuzzy_drawer.fuzzies) do
        ...
        if f.sprite ~= 63 then -- moving fuzzies
          ...
          -- move
          f.spd.x = (f.axis == "x" and mid(appr(f.spd.x,2*f.dir*f.scale,0.4*f.scale),f.l-f.x,f.r-f.x)) or (f.axis=="r+" and cos(-time()/2)*2) or (f.axis=="r-" and cos(time()/2)*2) or 0
          f.spd.y = (f.axis == "y" and mid(appr(f.spd.y,2*f.dir*f.scale,0.4*f.scale),f.l-f.y,f.r-f.y)) or (f.axis=="r+" and sin(-time()/2)*2) or (f.axis=="r-" and sin(time()/2)*2) or 0
          ...
          --eyes[#eyes+1]={f.x + (f.axis=="x" and 4+f.eye_offset or f.axis=="r+" and cos(-time()/2) or f.axis =="r-" and cos(time()/2) or 1), f.y + (f.axis=="y" and 4+f.eye_offset*.7 or f.axis=="r+" and sin(-time()/2) or f.axis=="r-" and sin(time()/2) or 0)}
          --eyes[#eyes+1]={f.x + (f.axis=="x" and 4+f.eye_offset or f.axis=="r+" and 4+cos(-time()/2)-cos((-time()-10)/2)*4 or 6), f.y + (f.axis=="y" and 4+f.eye_offset*.7 or f.axis=="r+" and 4+sin(-time()/2)-sin((-time()-10)/2)*4 or 5)}

          eyes[#eyes+1]={f.x + 4 + (f.axis=="x" and f.eye_offset or f.axis=="r+" and cos(-time()/2-3.141592)*4 or f.axis=="r-" and cos(time()/2-3.141592)*4 or -3), f.y + 4 + (f.axis=="y" and f.eye_offset*.7 or f.axis=="r+" and sin(-time()/2-3.141592)*4 or f.axis=="r-" and sin(time()/2-3.141592)*4 or -4)}
          eyes[#eyes+1]={f.x + 4 + (f.axis=="x" and f.eye_offset or f.axis=="r+" and cos(-time()/2+3.141592)*4 or f.axis=="r-" and cos(time()/2+3.141592)*4 or 2), f.y + 4 + (f.axis=="y" and f.eye_offset*.7 or f.axis=="r+" and sin(-time()/2+3.141592)*4 or f.axis=="r-" and sin(time()/2+3.141592)*4 or 1)}
          --eyes[#eyes+1]={f.x + 4 + cos(-time()/2+3.141592)*4, f.y + 4 + sin(-time()/2+3.141592)*4}
  ...
  draw=function(_ENV)
    if (#fuzzy_drawer.fuzzies!=0) then
      ...
      for b in all(fuzzy_drawer.fuzzies) do
        --if (b.sprite~=63) then
        --  eyes[eye]={appr(eyes[eye]~=nil and eyes[eye][1] or b.x,b.axis=="x" and b.x+2+2*b.dir or b.x,3),appr(eyes[eye]~=nil and eyes[eye][2] or b.y,b.axis=="y" and b.y+2*b.dir or b.y,3)}
        --end
        --if (b.sprite==63 and eyes[eye]==nil) eyes[eye]={b.x+(b.eyes[1]*(rnd"2"+1)+4),b.y+b.eyes[2]*(rnd"2"+1)+4}
        --eyes[#eyes+1]={b.x+b.eyes[1]*(rnd"2"+1)+4,b.y+b.eyes[2]*(rnd"2"+1)+4}
      ...
      for c in all(circs) do
        circfill(c[1],c[2],c[3],8)
      end
      -- draw fill
      for c in all(circs) do
        circfill(c[1],c[2],c[3]-1,3)
      end
      -- draw dots
      ...
      -- highlights
      for i=1,128 do
          circfill((rnd"128"+time()*(rnd"8"+8))%136-4,(rnd"128"+time()*(rnd"8"+8))%136-4,3,2)
      end
      ...
      for e in all(eyes) do
        --circfill(e[1],e[2],1,8)
        --pset(e[1],e[2],3)
        rectfill(e[1],e[2],e[1]+1,e[2]+1,8)
          --eyes=c[3]%40
          --circ(c[1]+(eyes%2)*2,c[2]+sin(eyes%2)*2,1,8)
          --circ(c[1]+cos(eyes/20)*-2,c[2]+sin(eyes/20)*-2.1,1,8)
      end
    end
  end
}
...
function _draw()
  ...
  -- draw objects - this has been moved above draw terrain
...
--@conf
--[[param_names = {}
autotiles = {{52, 54, 53, 39, 33, 35, 34, 55, 49, 51, 50, 48, 36, 38, 37, 72, [0] = 32}, {114, 116, 115, 69, 66, 68, 67, 101, 98, 100, 99, 85, 82, 84, 83, [0] = 117}, {40, 40, 40, 40, 58, 57, 40, 40, 42, 41, 40, 40, 40, 40, 40, 16, 56, 88, 103, 104, [0] = 40}}
]]
--@begin
--level table
--"x,y,w,h,exit_dirs"
...
mapdata={
  [3] = ""
}
...

...

...
__gff__

__map__

