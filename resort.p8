...
still_fuzz={
  init=function(_ENV)
    ...
    if (fget(tile_at((x/8)+1,y/8),6)) x+=4
    if (fget(tile_at((x/8)-1,y/8),6)) x-=4
    if (fget(tile_at(x/8,(y/8)+1),6)) y+=4
    if (fget(tile_at(x/8,(y/8)-1),6)) y-=4
    outline=false
    t=0
    if (mget(x/8,y/8)>=66 and mget(x/8,y/8)<=69) then
...
fuzz_spawn={
  ...
  update=function(_ENV)
    seed+=rnd"0.1"
    seed2+=rnd"0.1"
    if (player_here()) hidden=true player_was_here=true
    if (player_was_here and not player_here() and t==-1) t=5
    if (t>0) t-=1
    if (t==0) then -- create the actual fuzzies
      init_object(still_fuzz,x,y,63)
      --init_object(smoke,x-4,y+2,26)
      destroy_object(_ENV)
    end
  end,
  draw=function(_ENV)
    ...
    for i=0,1 do
      k=2+i*4
      for j=0,player_was_here and 1 or 2 do
        -- wiggle wiggle wiggle wiggle wiggle, yeah
        if (j>0) k+=mid(-1,round(sin((j/3)*(i==0 and seed or seed2 +time()/4))-0.4),1)
        pset(x+(dir_y~=0 and k or 0)+(1*dir_y)+(j*dir_x)+(dir_x==-1 and 8 or 0),y+(dir_x~=0 and k or 0)+(dir_y==1 and 7 or 0)-(j*dir_y)+(1*dir_x),1) -- shading
        pset(x+(dir_y~=0 and k or 0)+(j*dir_x)+(dir_x==-1 and 8 or 0),y+(dir_x~=0 and k or 0)+(dir_y==1 and 7 or 0)-(j*dir_y),2) -- line
      end
    end
  end
}
...
mapdata={
  [3] = ""
}
...

...

...

