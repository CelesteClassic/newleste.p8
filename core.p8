...
cassette_block={
  init=function(_ENV)
  		col=sprite-79
    blink=0
    timer=col*_g.cassette_time/2
				
				while right()<_g.lvl_pw-1 and tile_at(right()/8+1,y/8)==sprite-16 do 
      hitbox.w+=8
    end 
    while bottom()<_g.lvl_ph-1 and tile_at(x/8,bottom()/8+1)==sprite-16 do 
      hitbox.h+=8
    end
  ...
  update=function(_ENV)
  timer+=1
  local c_time=timer%_g.cassette_time

  solid_obj=not(player_here() or c_time<_g.cassette_time/2)
  blink=max(c_time>_g.cassette_time-3 and -.5,_g.cassette_time/2+3-c_time)
  end,
  
  	draw=function(_ENV)
  		local top,bottom,left,right=y-blink+.5, y+hitbox.h, x-blink, x+hitbox.w-.5+blink
  		if solid_obj then
  			rectfill(left,top,right,bottom+1,col)
  			rectfill(left,top,right,bottom-1,col*2+10)
  		 rect(left+1,top+1,right-1,bottom-2,col)
  		else
  			fillp(0b1100100100110110)
  			rectfill(x,y+2,right-blink,bottom+1,1)
  			fillp()
  		end
  end
}
