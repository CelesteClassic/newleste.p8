...
function draw_lighting()
  ...
  for a in all(objects) do
    if a.light then add(lights, a) end
  end
  
  for y=0,127 do
    local brkpts={}
    for a in all(lights) do
      for l=0,1 do
        local dy = y-a.y-cam_y+64
        local dx = sqrt(a.light+(300*l) - dy*dy)
        local bx = (a.x-cam_x+68)-dx
        local bx2 = (a.x-cam_x+68)+dx
        if dx~=0 then
          add_brkpt(brkpts, {x=flr(max(bx,0)),d=-1})
          add_brkpt(brkpts, {x=ceil(min(bx2,127)),d=1})
        end
      end
    end

    if #brkpts==0 then add(brkpts,{x=0,d=0}) end
    if brkpts[#brkpts].x ~= 127 then add(brkpts,{x=127,d=0}) end

    local lv=3
    local x=0

    for b in all(brkpts) do
      if lv <=3 and lv >= 1 and x~=b.x then
        fills[lv](x,b.x,y)
        x=b.x
      end
      lv += b.d
...
function add_brkpt(tbl,b)
  local iStart,iEnd,iMid,iState = 1,#tbl,1,0
  while iStart <= iEnd do
    iMid = flr((iStart+iEnd)/2)
    if b.x < tbl[iMid].x then
      iEnd,iState = iMid-1,0
    else
      iStart,iState = iMid+1,1
    end
  end
  add(tbl,b,iMid+iState)
end

function px_to_screen_addr(x, y, bias)
  return mid(0x6000, 0x6000+(y<<6)+bias(x>>1), 0x7fff)
end

function light_line(level)
  local lut_address = 0x4300+level*0x100
  return function(x1, x2, y)
    local lut_addr = lut_address
    x1=mid(0, x1, 127)
    x2=mid(0, x2, 127)
    y=mid(0, y, 127)
    local a_start, a_end = px_to_screen_addr(x1, y, ceil), px_to_screen_addr(x2-1, y, flr)
    if x1%2==1 then
      local addr = a_start-1
      local prev = @addr
      poke(addr, (prev & 0xf) | (@(lut_addr | @addr) & 0xf0))
    end
    for addr = a_start, a_end do
      poke(addr, @(lut_addr | @addr))
    end
    if x2%2==0 then
      local addr = a_end+1
      local prev = @addr
      poke(addr, (prev & 0xf0) | (@(lut_addr | @addr) & 0xf))
    end
  end
end

function light_none(x1, x2, y) end
...
function load_lighting_lut()
 for lv=1,3 do
  local addr=0x4300+(lv<<8)
  local sx=lv-1+120
  for c1=16,31 do
   local nc=sget(sx,c1)
   local topl=nc<<4
...

__label__
...

...

