...
{
	init=function(this) 
		this.underwater = false --REFLECTIONS
	...
	update=function(this)
		...
		local inputv = btn(k_down) and 1 or (btn(k_up) and -1 or 0) -- REFLECTIONS
   ...
   init_object(smoke,this.x,this.y)
  	...
  	if abs(this.spd.y) <= 0.15 then
   	gravity*=0.5
			...
			if not on_ground and not this.underwater then -- REFLECTIONS
...
fake_wall = {
	...
	update=function(this)
		...
		local hit = this.collide(player,0,0)
		local hitK = this.collide(kevin,0,0)
		if (hit~=nil and hit.dash_effect_time>0) or hitK~=nil then
			if hit~=nil then
...
kevin = {
	...
	update=function(this)
		...
		this.hitbox={x=-1,y=-1,w=(this.sizex*8)+2,h=(this.sizey*8)+2}
		...
		if hit~=nil and hit.dash_effect_time>0 then
			...
			--add to retrace list
			add(this.retrace, {x=this.x, y=this.y})
		...
		--back to normal hitbox
		this.hitbox={x=0,y=0,w=this.sizex*8,h=this.sizey*8}

		--backtrack
		if #this.retrace ~= 0 and this.movex == 0 and this.movey == 0 and this.retracetimer == 0 and
			abs(this.x - this.retrace[#this.retrace].x) <= 1 and abs(this.y - this.retrace[#this.retrace].y) <= 1 then
			this.x = this.retrace[#this.retrace].x
			this.y = this.retrace[#this.retrace].y
			this.spd.x = 0
			this.spd.y = 0
			this.retracetimer = 5
			shake=2
		...
		if this.retracetimer > 0 then
			this.retracetimer-=1
			if this.retracetimer==0 then
				del(this.retrace, this.retrace[#this.retrace])
				if #this.retrace ~= 0 then
					this.spd.x = sign(this.retrace[#this.retrace].x - this.x)
					this.spd.y = sign(this.retrace[#this.retrace].y - this.y)
				end
			end
		end
		...
		if this.hittimer > 0 then
			this.hittimer-=1
			if this.hittimer==0 then
				this.spd.x = this.movex
				this.spd.y = this.movey
			end
		end
		...
		--check collision
		if (this.is_solid(this.movex, this.movey) and (this.movex ~= 0 or this.movey ~= 0)) or 
			this.x + this.spd.x < 0 or
			this.x + this.spd.x > 129-this.hitbox.w or
			this.y + this.spd.y < 0 or
			this.y + this.spd.y > 129-this.hitbox.h then

			this.movex = 0
			this.movey = 0
			this.spd.x = 0
			this.spd.y = 0
			this.x = clamp(this.x, 0, 129-this.hitbox.w)
			this.y = clamp(this.y, 0, 129-this.hitbox.h)

			shake=5
			--add our current position to the retrace list so we trigger retracing
			add(this.retrace, {x=this.x, y=this.y})
		end
	...
	draw=function(this)
		--corners
		if this.movey < 0 or this.spd.y < 0 or this.movex < 0 or this.spd.x < 0 then pal(12, 7) end
		spr(66,this.x,this.y)
		pal(12, 12)
		if this.movey < 0 or this.spd.y < 0 or this.movex > 0 or this.spd.x > 0 then pal(12, 7) end
		spr(66,this.x+(8*(this.sizex-1)),this.y, 1, 1, true, false)
		pal(12, 12)
		if this.movey > 0 or this.spd.y > 0 or this.movex < 0 or this.spd.x < 0 then pal(12, 7) end
		...
		for x=1,this.sizex-2 do
			if this.movey < 0 or this.spd.y < 0 then pal(12, 7) end
		...
		for y=1,this.sizey-2 do
			if this.movex < 0 or this.spd.x < 0 then pal(12, 7) end
		...
		--face
		if this.movex==0 and this.movey==0 and this.spd.x==0 and this.spd.y==0 then
			spr(68, this.x+((this.sizex-1)*8)/2, this.y+((this.sizey-1)*8)/2)
		else
			spr(69, this.x+((this.sizex-1)*8)/2, this.y+((this.sizey-1)*8)/2)
		end
	end
}
add(types,kevin)
...
feather = {
	...
	update=function(this) 
		if this.spr==98 then
			...
			if hit~=nil then
				...
				this.x=hit.x
				this.y=hit.y
				this.spd.x=hit.spd.x
				this.spd.y=hit.spd.y
				shake=8
				destroy_object(hit)
			end
		elseif this.timer>0 then
		...
		if this.spawntimer > 0 then
			this.spawntimer-=1
			this.spd.x=appr(this.spd.x, 0, 0.7)
			this.spd.y=appr(this.spd.y, 0, 0.7)
			if this.spawntimer==0 then
				init_object(smoke,this.x,this.y)
				local newP = init_object(feather_player, this.x, this.y)
				newP.movedir = this.spawndir
			end
		end
	...
	draw=function(this)
		...
		if this.spr==98 then
			local sproff = 0
			if flr(this.sprtimer%5)==0 or flr(this.sprtimer%5)==4 then sproff = 1 end
			if flr(this.sprtimer%5)==2 then sproff = 2 end
			spr(this.spr+sproff,this.x,this.y, 1, 1, this.sprtimer%5<2)
		end
		--draw start of orb
		if this.spawntimer > 0 then
...
feather_player={
	init=function(this)
		--constants
		this.max_movespeed=1.5
		this.max_lifetime=45
		...
		this.tail={}
		for i=0,15 do
			add(this.tail,{x=this.x,y=this.y,size=max(0,min(1,8-i))})
		end
		this.particles={}
	end,
	update=function (this)
		...
		if inputx ~= 0 or inputy ~= 0 then
			this.movedir = appr_circ(this.movedir, atan2(inputx, inputy), 0.04)
		end

		this.spd.x = cos(this.movedir)
		this.spd.y = sin(this.movedir)

		--speed up if holding button
		if inputx ~= 0 or inputy ~= 0 then
			this.spd.x *= this.max_movespeed + clamp((this.lifetime-80)/10, 0, 10)
			this.spd.y *= this.max_movespeed + clamp((this.lifetime-80)/10, 0, 10)
		end
		...
		if this.bouncetimer==0 then
			...
			elseif this.is_solid(2, 0) or this.is_solid(-2, 0) then
				if this.movedir < 0.5 then
					this.movedir = 0.5-this.movedir
				else
					this.movedir = 1.5-this.movedir
				end
				this.bouncetimer = 2
				init_object(smoke, this.x, this.y)
			end
		...
		if this.lifetime > 0 then
			...
			if this.lifetime==0 then
				...
				init_object(player, this.x, this.y)
				shake=2
				convert_particles(this.particles)
				destroy_object(this)
			end
		end
		...
		if btn(k_dash) then
			init_object(smoke, this.x, this.y)
			local newP = init_object(player, this.x, this.y)
			newP.dash=true
			convert_particles(this.particles)
			destroy_object(this)
		end
		...
		--feather collide
		local hit = this.collide(feather, 0, 0)
		if hit~=null and hit.spr==98 then
			this.lifetime=this.max_lifetime
			psfx(6)
			init_object(smoke,this.x,this.y)
			hit.spr=0
		...
		local particle = {x=this.x+(rnd(8)-4), y=this.y+(rnd(8)-4), xspd=-this.spd.x/2, yspd=-this.spd.y/2, life=10+flr(rnd(5))}
		particle.xspd -= (this.x-particle.x)/4
		particle.yspd -= (this.y-particle.y)/4
		add(this.particles, particle)
	end,
	draw=function(this)

		foreach(this.particles, function(p)
			rect(p.x+4, p.y+4, p.x+4, p.y+4, 10)
			p.x+=p.xspd
			p.y+=p.yspd
		...
		local last={x=this.x+4.5,y=this.y+4.5}
...
function appr_circ(value,target,amount)
	if abs(target-value) > abs((target+1) - value) then
		return appr(value, target+1, amount)%1
	elseif abs(target-value) > abs((target-1) - value) then
		return appr(value, target-1, amount)%1
	else
		return appr(value, target, amount)%1
	end
end
...
water = {
	...
	init=function(this)
		this.tiles={}
		init_water(this.x, this.y, this.y, this)
		this.timer=0
	...
	update=function(this)
		...
		for i in all(this.tiles) do
			this.hitbox.x = i.x-this.x
			this.hitbox.y = i.y-this.y
			local other = this.collide(player, 0, 0)
			if other ~= null then
				if this.hasplayer==false then
					init_object(smoke, other.x, other.y)
					poke(0x5f41, 0xf)
					poke(0x5f43, 0xf)
				end
				this.player=other
	...
	draw=function(this)
		...
		for i in all(this.tiles) do
			for py=i.y+off,i.y+off+3,1 do
				local row = py*64
				for px=i.x,i.x+7,2 do
					local addr = abs(0x6000 + row + (px/2))
					poke(addr, (peek(addr+1) << 4) | (peek(addr) >> 4))
				end
			end
			if i.y == this.y then
				for bx=0, 7 do
		...
		for i in all(this.tiles) do
			...
			for py=i.y+top,i.y+7 do
				local row = py*64
				for px=i.x,i.x+7,2 do
					local addr = 0x6000 + row + (px/2)
					local scol = peek(addr)
					local col = (shift_pal((scol & 0xf0) >> 4) << 4) | shift_pal(scol&0xf)
					poke(addr, col)
				end
			end
		end
...
function init_water(sx, sy, maxy, obj)
	add(obj.tiles, {x=sx, y=sy})
	if sx+8 < 127 and not solid_at(sx+8, sy, 8, 8) and not contains(obj.tiles, {x=sx+8, y=sy}) then
		init_water(sx+8, sy, maxy, obj)
	end
	if sx-8 > -1 and not solid_at(sx-8, sy, 8, 8) and not contains(obj.tiles, {x=sx-8, y=sy}) then
		init_water(sx-8, sy, maxy, obj)
	end
	if sy+8 < 127 and not solid_at(sx, sy+8, 8, 8) and not contains(obj.tiles, {x=sx, y=sy+8}) then
		init_water(sx, sy+8, maxy, obj)
	end
	if sy-8 >= maxy and not solid_at(sx, sy-8, 8, 8) and not contains(obj.tiles, {x=sx, y=sy-8}) then
		init_water(sx, sy-8, maxy, obj)
	end
	sort(obj.tiles)
end
...
function shift_pal(col)
	local addr = 0xe0c + flr((col%8)/2) + flr(col/8)*64
	if col%2 == 0 then return peek(addr) & 0xf end
...
bumper = {
	...
	update=function(this)
		if this.hittimer <=0 then
			local hit=this.collide(player,0,0)
			if hit ~=nil then
				init_object(smoke,hit.x,hit.y)
				psfx(16)
				local dx = (this.x+8)-hit.x
				local dy = (this.y+8)-hit.y
				if abs(dx) > abs(dy) then
					hit.spd.x = 3.5*(-cos(0.90)*sign(dx))
					hit.spd.y = 3.5*(-sin(0.90))
				else
					local angle = atan2(dx,dy)
					hit.spd.x = 3*(-cos(angle))
					hit.spd.y = 3*(-sin(angle))
				end
				hit.dash_time=0
				hit.djump=max_djump
				this.hittimer=20
	...
	draw=function(this)
		if this.hittimer>0 then
			...
			if this.hittimer > 16 then
				circ(this.x+8, this.y+8, 6+(20-this.hittimer), 7)
				circfill(this.x+8, this.y+8, 5+(20-this.hittimer), 1)
				if this.hittimer > 18 then
					rectfill(this.x+4, this.y-1, this.x+12, this.y+17, 7)
					rectfill(this.x-1, this.y+4, this.x+17, this.y+12, 7)
				end
			end
		end
		...
		if this.hittimer == 1 then
			circfill(this.x+8, this.y+8, 4, 6)
		end
	end
}
add(types, bumper)
...
function _draw()
	...
	-- draw objects
	foreach(objects, function(o)
		if o.type~=platform and o.type~=big_chest and o.type~=water then --REFLECTIONS
			draw_object(o)
		end
	end)
...

...
__gff__

__map__

