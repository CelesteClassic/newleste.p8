...
levels={
  "0,0,1.4375,1,0b0001,1,bg tilling at end could be tweaked. berry feels like it's on the casual route and updashing in the center to skip it feels unsatisfying",
  "1.4375,0,2,1,0b0010,1,needs a bit of polish/slightly more interesting terrain; last dream block should be wider. start should be 1-2 block(s) lower to make diags into the first dream block more friendly and to make walljumping to get the token feel more necessary",
  "3.4375,0,1.5625,1,0b0010,0,right-facing spikes feel like they're a weird distance from the pillar but could just be me. Straight terrain lines could be interesting'd up, and second dash crystal feels oddly placed (could be 1 tile left.)",
  "5,0,1,1,0b0010,1,maybe slightly modify block target and potentially lower top-left platform by 1 tile",
  "6,0,1,1,0b1000,1,0,left area doesn't really feel like an exit. this might be the intent (for it to feel kinda like an abandoned room type thing); but feels weird. also a little boring",
  "0,1,2.1875,1,0b1000,0,needs to be retiled; a bit big/empty rn could be compactified; not sure about cam trigger", -- summary: needs shrunk and/or reworked
  "2,3,1,1,0b0001,2,left side could have something to make it not look like a block chute. dreamblock feels uneccessary. otherwise quite fun",
  "0,2,1,2,0b0001,3,left side needs polish; lags but might not w/ new dreamblocks; camera needs to shift down so you can see where the fuck you're going. moveblock slot looks a little 'cheap' but not sure how to fix",
  "5,1,1,1,0b0001,0,left dreamblock looks a bit smol; also could have spikes on top so you can't land which feels like something you shouldn't be able to do. also badly positioned for updashing, left side could be reworked a tiny bit",
  "1,3,1,1,0b0010,0,left side needs slight retiling; block should go all the way left (or go into a wall.) movement is fun but dreamblocks could be moved a little; they feel a little cramped atm",
-- really hating on the left-sides today lol
  "3,1,1,1,0b0001,2,terrain is a bit weird and can be tweaked; berry is maybe a tad too tight?", -- berry felt fine to me
  "6,1,1,1,0b0001,3,movement is fun but it's a bit empty; also badeline number feels a little excessive. 2 would be fine",
  "4,1,1,1,0b0001,3,it's too easy to skip the whole right side; also dreamblocks feel strangely positioned in relation to terrain",
  "7,0,1,4,0b0010,5,should use a cam trigger so falls are less blind; 2nd berry is too hard (add a roundabout?); badeline num seems maybe excessive; probably need to nerf length/ending; move block should go somewhere less in the the way/ending is a bit wacky and tight. also the spikes on the right wall in the middle suck",
  "1,2,1,1,0b0010,0,0,2 wj's at the top left feels uneccessary but you can't reach with 1. needs reorganized a little; also needs spikes on the left (and maybe right) side of the stalactite; or remove the second dash crystal",
  "3,2,1,1,0b0001,1,lacks substance (!!!)",
  "2,2,1,1,0b0001,1,might be better as a non badline level",
  "4,2,3,1,0b0010,2,berry exit needs a nerf; also can cheese berry by just tanking a death; ending dream block layout can maybe be more interesting. could use cam triggers at the end so it's less blind, also dash crystal and general middle section layout is confusing",
  "3.875,5.375,1,3,0b0100,4,like this less than the 1st vertical chase level. maybe should split into 2+ levels (send back to designer?); ending of long dreamblock should be more lenient. first walljump should be communicated more clearly (maybe move the second wall right 1 tile); could just be compacted as the start and very end were very fun",
  "1.125,5.375,2.375,1.25,0b0001,1,wayyyy to open. needs significant rework; also right top dreamblock needs to be a little wider. unclear where you need to go. also what's up with the berry just floating there?",
  "0,6.8125,2.4375,1.25,0b0001,3,too gimmicky. start is a fun puzzle but the bottom is just really meh. berry is also where i'd go back right even if i wasn't doing hundo",
  "0.125,8.25,1,1,0b0001,1,feels like a concept level but works surprisingly well. just need to make it a little bit more interesting; and tweak a little to make less tight/more clear. big berry potential", -- where?
  "1.1875,8.25,1,1.0625,0b0010,1,gimmick level; fun but not really all that usable. similar, smaller concept could be used for a berry/secret somewhere though.",
  "0.125,9.375,2,1,0b0010,0,needs tweaks ofr both tiling and terrain (and spacing). overall good tho", -- +1
  "3.5625,9.375,1,1,0b0001,0,mirror room- needs mirror; terrain isn't all that great though. Rework",
  "4.6875,9.3125,2,1,0b1000,0,good concept but needs redone imo. spacing is weird; camtriggers are weird and there's too many landing spots",
  "6.8125,9.3125,1,1,0b0001,0,non berry route kinda lacking(replace berry with token?);  pretty good concept -  but missing some polish; dash crystal above berry is too high and things don't quite line up",
  "0.0625,10.5625,1,2,0b0010,1,very fun but needs retile and slight polish; token block is weird",
  "1.3125,10.5,1,1,0b0001,1,0,good but room is a little boring. also why the hell is the token block 6 blocks up from the ground",
  "2.5625,10.5,1,2,0b1000,1,leadup and cam trigger feel awesome imo but the top half of the level in general isn't very fun and doesn't deserve the buildup; exit isn't telegraphed well at all; berry's a little weird; the spikes that the spring pushes you into don't actually look like you're going to hit them which is just plain stupid. they should extend up 2 more tiles",
  "3.6875,10.4375,1,1,0b0010,3,fun concept but a little boring atm",
  "0.0625,12.6875,2,1,0b1001,0,has potenetial but needs spacing polish (mainly in the first half); tiling around berry feels a tad weird.",
  "0.125,13.8125,3,1,0b0100,0,spikes on ground need fixed or sparky's going to have a meltdown; tiling of first blue block looks like a secret breakable block and should be tweaked; need to figure out what's gonna happen to the berry if doing level duplication stuff",
