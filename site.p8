...
token = {
  ...
  draw=function(this)
    if collected_tokens~=total_tokens then
      this.timer+=1
      this.timer%=20
      this.spr=token.frames[flr(this.timer/5)+1]
      this.flipx=flr(this.timer/5)==3
    end
    palt(0,false)
    palt(8,true)
...
token_block={
  ...
  update=function(this)
    if this.shaketime>0 then
      if this.shaketime%2==0 then
        this.draw_x=this.x+rnd(2)-1
        this.draw_y=this.y+rnd(2)-1
      end
    else
      this.draw_x,this.draw_y=this.x,this.y
    end
    if this.state==0 and total_tokens==collected_tokens then
      this.state=1
      this.shaketime=20
    elseif this.state==1 then
      if this.shaketime>0 then
    ...
    elseif this.state==2 then
      local yspd =  mid(0.8,abs(this.y-this.target.y)/8,0.3)
      local xspd =  mid(0.8,abs(this.x-this.target.x)/8,0.3)
      local xdir = sign(this.target.x-this.x)
      local ydir = sign(this.target.y-this.y)
      local bothit =  this.collide(player,0,2)
      local sidehit = this.collide(player,xdir,0)
      local tophit = this.collide(player,0,-1)
      if ydir == 1 and bothit~=nil then
        bothit.move_y(flr(this.rem.y+yspd*ydir + 0.5))
      end
      if sidehit~=nil then
        sidehit.move_x(flr(this.rem.x+xspd*xdir + 0.5),0)
      end
      if ydir == -1 and tophit~=nil then
        tophit.move_y(flr(this.rem.y+yspd*ydir + 0.5))
      end
      this.move(xspd*xdir,yspd*ydir)
      if tophit~=nil then
        if ydir==1 then
          tophit.move_y(this.y-this.last.y)
        end
        tophit.move_x(this.x-this.last.x,1)
      end
  ...
  draw=function(this)
    spr(66,this.draw_x,this.draw_y,2.0,2.0)
  end
}
add(types,token_block)
--- </token> ---
...
function init_object(type,x,y)
  ...
  obj.is_solid=function(ox,oy)
    ...
    --- <tokens> ---
    or obj.check(token_block,ox,oy)
  --- </tokens> ---
  end
...
function load_room(x,y)
  ...
  for tx=0,15 do
    for ty=0,15 do
      ...
      --- <token> ---
      elseif tile==66 then
        init_object(token_block,tx*8,ty*8).target.x=tx*8-24
      elseif tile==67 then
        init_object(token_block,tx*8-8,ty*8).target.y=ty*8-24
      elseif tile==82 then
        init_object(token_block,tx*8,ty*8-8).target.y=ty*8+16
      elseif tile==83 then
        init_object(token_block,tx*8-8,ty*8-8).target.x=tx*8+16
      --- </token> ---
      else
        foreach(types,
          function(type)
...
__gfx__

...

...
__gff__

...

